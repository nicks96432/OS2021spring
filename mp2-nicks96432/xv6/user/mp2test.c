...
{
  printf("mp2test starting\n");
  if (syscall_test() && fork_test())
    printf("mp2test: all tests succeeded\n");
  exit(0);
}
...
err(char *why)
{
  printf("mp2test: %s failed: %s, pid=%d\n", testname, why, getpid());
  exit(1);
}
...
syscall_test(void)
{
  ...
  printf("test mmap bare: PASS\n");
  
  testname = "mmap lazy";
  printf("test mmap lazy\n");
  ...
  printf("test mmap lazy: PASS\n");

  testname = "munmap bare";
  printf("test munmap bare\n");
  
  // unmap the rest of the mapped memory.
  if (munmap(p+PGSIZE*2, PGSIZE) == -1)
    err("munmap (4)");

  printf("test munmap bare: PASS\n");
  
  testname = "munmap exit";
  printf("test munmap exit\n");
...
fork_test(void)
{
  ...
  testname = "shared virtual memory";
  printf("test shared virtual memory\n");
  ...
  if(status != 0){
    printf("test shared virtual memory: FAIL\n");
    exit(1);
  }
  ...
  printf("test shared virtual memory: PASS\n");
  return 1;
}
