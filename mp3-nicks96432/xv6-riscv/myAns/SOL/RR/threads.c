...
static const int rr_time_quantum = 3;
static int id = 1;
...
void dispatch(void)
{
	...
	{
		// If remain_execution_time is smaller than time_slot_size,
		// we interrupt the thread after remain_execution_time ticks.
		int next_time = (__time_slot_size >= current_thread->remain_execution_time)
							? current_thread->remain_execution_time
							: __time_slot_size;
		// after next_time ticks, my_thrdstop_handler will be called.
...
void schedule(void)
{
	...
	else
	{
		...
		else
		{
			...
			if (current_thread->quantum == rr_time_quantum)
			{
