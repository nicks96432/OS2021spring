...
// Choose the next thread based on remain_execution_time (the shortest one).
// If remain_execution_time is the same, choose the thread arived earlier.
// The arrival order is determined by thread->ID (the smaller the earlier).
// Execute the next thread when (1) current thread exits. (2) current thread yields.
// 								(3) current thread has been executedtime_slot_size ticks.
void schedule(void)
{
	if (is_thread_start == 0)
	{
		...
		{
			if (t->remain_execution_time < next->remain_execution_time ||
				(t->remain_execution_time == next->remain_execution_time && t->ID < next->ID))
				if (!t->is_exited)
					next = t;
			t = t->next;
		}
	...
	{
		int is_exited = current_thread->is_exited;
		int is_yield = current_thread->is_yield;
		int prev_time = current_thread->prev_time;
		if (is_exited || is_yield || uptime() - prev_time >= __time_slot_size)
		{
			...
			current_thread = next;
			current_thread->prev_time = uptime();
		}
	}
}
